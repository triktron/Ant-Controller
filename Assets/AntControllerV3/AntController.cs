...
public class AntController : MonoBehaviour
{
    ...
    private Vector3 _RestingCenterOffset = Vector3.zero;
    ...
    private void Start()
    {
        ...
        _RestingCenterOffset = transform.InverseTransformVector(GetTargetCenter() - transform.position);
    }
    ...
    private void UpdateSpeed()
    {
        ...
        var center = _RestingCenterOffset - GetTargetCenter();
        center += transform.up * _FloorHeight;

        Debug.DrawRay(GetTargetCenter(), transform.up * 2, Color.green);
        Debug.DrawRay(GetTargetCenter() - transform.TransformVector(_RestingCenterOffset), transform.up * 1, Color.magenta);
        Debug.DrawRay(transform.position, transform.up * 2, Color.red);

        transform.position = Vector3.MoveTowards(transform.position, GetTargetCenter() - transform.TransformVector(_RestingCenterOffset), Time.deltaTime * 100f);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawSphere(targetcenter, .05f);
        Gizmos.DrawRay(targetcenter, GetTargetNormal() * .4f);
    }
}
